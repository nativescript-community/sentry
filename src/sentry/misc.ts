import { EnvelopeItem, Exception } from '@sentry/types';
...
export function isHardCrash(payload: EnvelopeItemPayload): boolean {
    const values: Exception[] = typeof payload !== 'string' && 'exception' in payload && payload.exception?.values ? payload.exception.values : [];
