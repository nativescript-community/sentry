import type { Client, Event, Integration, MeasurementUnit, Measurements, Span } from '@sentry/core';
import { debug, timestampInSeconds } from '@sentry/core';
import { AsyncExpiringMap } from '../../utils/AsyncExpiringMap';
import { isRootSpan } from '../../utils/span';
import { NATIVE } from '../../wrapper';

// Use simplified frames response from our NATIVE wrapper
interface NativeFramesResponse {
    totalFrames: number;
    slowFrames: number;
    frozenFrames: number;
}
...
export const nativeFramesIntegration = (): Integration => {
    ...
    const _spanToNativeFramesAtEndMap: AsyncExpiringMap<string, NativeFramesResponseWithTimestamp | null> = new AsyncExpiringMap({ ttl: END_FRAMES_TIMEOUT_MS });
    ...
    return new Promise<NativeFramesResponse>((resolve, reject) => {
        ...
        NATIVE.fetchNativeFrames()
            .then((value) => {
                ...
                resolve(value as NativeFramesResponse);
            })
            .then(undefined, (error) => {
                if (settled) {
                    return;
