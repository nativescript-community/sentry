...
import { SPAN_STATUS_ERROR, debug, getSpanDescendants, spanToJSON } from '@sentry/core';
...
function getMeaningfulChildSpans(span: Span): Span[] {
    ...
    return children.filter(
        (child) => child.spanContext().spanId !== span.spanContext().spanId && spanToJSON(child).op !== 'ui.load.initial_display' && spanToJSON(child).op !== 'navigation.processing'
    );
}
 ...
 */
function discardEmptyNavigationSpan(client: Client | undefined, span: Span | undefined, shouldDiscardFn: (span: Span) => boolean, onDiscardFn: (span: Span) => void): void {
    if (!client) {
 ...
 */
export const ignoreEmptyRouteChangeTransactions = (client: Client | undefined, span: Span | undefined, defaultNavigationSpanName: string, isSpanStillTracked: () => boolean): void => {
    discardEmptyNavigationSpan(
        client,
        span,
        // Only discard if:
        ...
        (span) => {
            ...
            return spanJSON.description === defaultNavigationSpanName && !spanJSON.data?.['route.name'] && isSpanStillTracked();
        },
 ...
 */
export const cancelInBackground = (_client: Client, _span: Span): void => {
    // Platform-specific AppState handling has been removed to keep this module
    // platform-agnostic. If you need background cancellation behavior, provide
    // a platform integration that listens for background events and cancels
    // the span explicitly (e.g. call `span.setStatus(...)` and `span.end()`).
    debug.warn('cancelInBackground is disabled: AppState handling removed from this platform-agnostic module.');
};
